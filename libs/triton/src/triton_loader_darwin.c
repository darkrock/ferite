...
#ifndef TRITON_AS_COMPONENT
#include "../../../config.h"
#endif

#include "triton.h"

#if (defined(USING_DARWIN) || defined(USE_DARWIN)) && !defined(USING_SNOWLEOPARD)
...
LIB_SYM
{
    ...
    NSSymbol NSSymbol;
    void *address;
    char *nsym = malloc(strlen(symbol)+2);

    strcpy(nsym,"_");
    strcat(nsym,symbol);

    DEBUG_PRINT2("libdl: dlsym(%p,%s) -> ", handle, nsym);
   ...
   */
    ...
    {
        if(NSIsSymbolNameDefined(nsym) == TRUE)
        {
            NSSymbol = NSLookupAndBindSymbol(nsym);
   ...
   */
    ...
    while(p != NULL)
    {
        if(dlopen_handle == p)
        {
            NSSymbol = NSLookupSymbolInModule(p->module, nsym);
            if(NSSymbol != NULL)
            {
                address = NSAddressOfSymbol(NSSymbol);
                dlerror_pointer = NULL;
